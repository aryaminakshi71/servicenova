import { recordResilienceEvent } from '../../../server/observability';
import { nowMs } from '../../../server/test-controls';
import { injectTraceHeaders, withSpan } from '../../../server/tracing';
...
async function fetchWithPolicy(
	...
	const now = nowMs();
	const tenantId = currentTenantId()?.trim() || 'default';

	if (isCircuitOpen(state, now, config.cooldownMs)) {
		recordResilienceEvent({
			tenantId,
			type: 'integration_circuit_short_circuit',
			service,
			operation,
		});
		throw new Error(
	...
	while (attempt < maxAttempts) {
		...
		try {
			const response = await withSpan(
				`integration.${service}.${operation}`,
				{
					...
					'servicenova.tenant_id': tenantId,
					'net.peer.name': new URL(url).host,
				},
				async (span) => {
					const response = await fetchWithTimeout(
						...
						{
							...init,
							headers: injectTraceHeaders(init.headers),
						},
						config.timeoutMs,
					);
		...
		if (!wasOpen && state.openedAt !== null) {
			recordResilienceEvent({
				tenantId,
