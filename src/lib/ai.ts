import { isAIConfigured, requestStructuredAi } from './platform-ai-client';
...
export async function optimizeDispatch(input: {
	...
	const fallback = {
		assignments: input.jobs.slice(0, 3).map((j, i) => ({
			jobId: j.id,
			technicianId:
				input.technicians[i % input.technicians.length]?.id || 'TECH-001',
			route: [j.id],
		})),
		totalDistance: 45.2,
	};
	...
	return {
		assignments: aiResult.assignments || fallback.assignments,
		totalDistance: toNumber(aiResult.totalDistance, fallback.totalDistance),
	};
}
...
export async function predictMaintenance(input: {
	...
	return {
		...
		predictedFailureDays: toNumber(
			result.predictedFailureDays,
			fallback.predictedFailureDays,
		),
...
export async function analyzeTechnicianPerformance(input: {
	...
	const fallback = {
		score: Math.round(
			(input.avgRating / 5) * 100 * 0.7 + (100 - input.responseTime) * 0.3,
		),
...
export async function generateServiceReport(input: {
	...
	return {
		summary: (aiResult.summary as string) || fallback.summary,
		recommendations: toStringArray(
			aiResult.recommendations,
			fallback.recommendations,
		),
		followUp: (aiResult.followUp as string) || fallback.followUp,
	};
}
