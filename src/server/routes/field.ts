...
import {
	...
	getDriftAlertAcknowledgement,
	listIncidentTimelineEvents,
...
const onboardingUpdateSchema = z
	.object({
		selectedJob: z.boolean().optional(),
		intelligenceRun: z.boolean().optional(),
		dispatchOptimized: z.boolean().optional(),
		automationCycle: z.boolean().optional(),
	...
	.refine(
		(value) =>
			...
			value.intelligenceRun !== undefined ||
			value.dispatchOptimized !== undefined ||
...
const driftAlertAcknowledgeSchema = z.object({
	owner: z.string().min(1).max(120).optional(),
	slaDueAt: z.string().min(10).max(80),
...
async function enrichDriftAlertsWithAcknowledgements(
	tenantId: string,
	alerts: ReturnType<typeof getWorkOrderIntelligenceDriftAlerts>,
) {
	return Promise.all(
		alerts.map(async (alert) => {
			const acknowledgement = await getDriftAlertAcknowledgement(
				tenantId,
				alert.id,
			);
			return {
...
async function runAutoDisruptionSweep(
	...
	lastAutoDisruptionSweepAt.set(tenantId, nowMs());
...
fieldRoutes.get("/dashboard/onboarding", async (c) => {
	...
	const auth = getAuth(c);
	return c.json({
		onboarding: await getDashboardOnboardingState(auth.tenantId, auth.userId),
	});
});
...
fieldRoutes.post("/dashboard/onboarding", async (c) => {
	...
	const auth = getAuth(c);
	const onboarding = await updateDashboardOnboardingState(
		auth.tenantId,
		auth.userId,
		parsed.data,
	);
...
fieldRoutes.post("/dispatch/disruptions", async (c) => {
	...
	await appendIncidentTimelineEvent(auth.tenantId, {
		...
		actor: payload.actor,
		context: {
			disruptionType: result.type,
			impactedJobs: result.impactedJobIds.length,
			reassignedJobs: result.reassignedJobIds.length,
			queuedJobs: result.queuedJobIds.length,
			blockedJobs: result.blockedJobIds.length,
		},
	});
...
fieldRoutes.post("/dispatch/optimize", async (c) => {
	...
	if (parsed.data.async) {
		...
		await appendIncidentTimelineEvent(auth.tenantId, {
			...
			context: {
				backgroundJobId: job.id,
				includeAssigned: parsed.data.includeAssigned ?? false,
			},
		});
	...
	await appendIncidentTimelineEvent(auth.tenantId, {
		...
		context: {
			...
			unresolvedJobs: result.unassignedJobIds.length,
		},
	});
...
fieldRoutes.post("/ops/automation/run-cycle", async (c) => {
	...
	await appendIncidentTimelineEvent(auth.tenantId, {
		...
		context: {
			...
			sloBreached: metrics.sloBreached,
		},
	});
...
fieldRoutes.get("/intelligence/drift-alerts", async (c) => {
	...
	return c.json({
		alerts: await enrichDriftAlertsWithAcknowledgements(auth.tenantId, alerts),
	});
});
...
fieldRoutes.post(
	...
	async (c) => {
		...
		const body = await c.req.json().catch(() => ({}));
		const parsed = driftAlertAcknowledgeSchema.safeParse(body);

		if (!parsed.success) {
			return validationError(c, parsed.error.issues);
		}

		const parsedDueAt = new Date(parsed.data.slaDueAt);
		...
		const actor = parsed.data.actor ?? auth.userId;
		const acknowledgement: DriftAlertAcknowledgement = {
			alertId,
			owner: parsed.data.owner ?? actor,
			acknowledgedBy: actor,
			acknowledgedAt: nowIso(),
			slaDueAt: parsedDueAt.toISOString(),
...
export function resetFieldRouteStateForTests() {
	lastAutoDisruptionSweepAt.clear();
	resetFieldDashboardStateForTests();
}
